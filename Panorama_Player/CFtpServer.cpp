...
#if 1
							char * pszTempName = (char *)malloc(strlen( fi->pszName ) + 2);
							memset(pszTempName,0,strlen( fi->pszName ) + 2);
							memcpy(pszTempName,fi->pszName,strlen( fi->pszName ));
							pszTempName[strlen( fi->pszName )] = '\n';

							if( !pClient->AddToListBuffer( pTransfer,pszTempName, strlen( pszTempName ), pBuffer, &nBufferPos, uiBufferSize ) ){
								break;
							}
#else
